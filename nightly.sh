...
# Function to fetch the latest branch with a specific pattern, excluding 'nightly', or default to master if not found
fetch_latest_branch() {
  local repo_url="$1"
  local pattern="${2:-rhoai}"
  local exclude_pattern="nightly"  # Pattern to exclude from search
  local branch=$(git ls-remote --heads "$repo_url" | grep "$pattern" | grep -v "$exclude_pattern" | awk -F'/' '{print $NF}' | sort -V | tail -1)
  if [ -z "$branch" ]; then
    echo "No branch matching the pattern '$pattern' and not containing '$exclude_pattern' found. Defaulting to 'master'."
    branch="master"
  fi
  echo "$branch"
}
